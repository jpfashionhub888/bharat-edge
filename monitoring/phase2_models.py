...
def train_base_models(X, y, n_splits=5):
    ...
    print(f"  {len(X)} samples | {X.shape[1]} features | {n_splits}-fold TSS CV")
    print(f"{'='*50}")
...
def soft_vote_proba(
    ...
    """
    Manual soft voting.
    FIXED: Forces n_jobs=1 to avoid joblib/Python 3.14 crash.
    """
    all_probs = []
    for name, model in base_models.items():
        try:
            # ✅ Force single thread before every prediction
...
def train_full_ensemble(
    ...
    # 2. Train base models on FULL data
    base_models, cv_scores, scaler = train_base_models(X, y, n_splits)

    # 3. Hold-out evaluation (last 20%)
    ...
    v_rep   = classification_report(
                  ...
                  target_names=['DOWN','UP'],
                  output_dict=True, zero_division=0)
    v_up_p  = v_rep['UP']['precision']
    v_up_r  = v_rep['UP']['recall']
    v_up_f1 = v_rep['UP']['f1-score']

    print(f"  {'SOFT VOTING':<22} {v_acc:>7.4f} {v_auc:>7.4f} "
          f"{v_up_p:>7.4f} {v_up_r:>7.4f} {v_up_f1:>7.4f}  ⭐")
...
def _save_models(base_models, scaler, feature_names):
    ...
    for name, m in base_models.items():
        pickle.dump(
            m, open(os.path.join(MODEL_DIR, f"{name}.pkl"), 'wb'))
        print(f"     ✅ {name}.pkl")
    pickle.dump(
        scaler,
...
def load_all_models() -> dict:
    ...
    try:
        ...
        for name in ['xgboost','lightgbm','random_forest','extra_trees']:
            base_models[name] = pickle.load(
                open(os.path.join(MODEL_DIR, f"{name}.pkl"), 'rb'))
            # ✅ Force single thread on load
...
def predict_portfolio(symbols, ensemble, **market_kwargs) -> pd.DataFrame:
    ...
    for sym in symbols:
        ...
        r = predict_live(symbol=sym, ensemble=ensemble, **market_kwargs)
        if 'error' in r:
            print(f"⚠️  {r['error']}")
        else:
            emoji = "🟢" if r['direction'] == 'UP' else "🔴"
            print(f"{emoji} {r['signal']}  conf={r['confidence']:.1f}%")
            results.append(r)
    ...
    print(f"  {'#':<3} {'Symbol':<14} {'Signal':<14} "
          ...
          f"{'XGB':>7} {'LGB':>7} {'RF':>7} {'ET':>7} {'AVG':>7}")
    print(f"  {'-'*72}")

    for i, row in df.iterrows():
        p     = row.get('individual_probs', {})
        emoji = "🟢" if row['direction'] == 'UP' else "🔴"
        votes = f"{row.get('up_votes',0)}U/{row.get('down_votes',0)}D"
        print(
            f"  {i+1:<3} {row['symbol']:<14} {row['signal']:<14} "
            f"{row['confidence']:>6.1f}% {emoji:>3} {votes:>7}  "
...
if __name__ == "__main__":
    ...
    for rank, (_, row) in enumerate(imp.head(15).iterrows(), 1):
        bar = "█" * int(row['avg'] * 25)
        print(f"  {rank:>2}. {row['feature']:<25} {row['avg']:.4f}  {bar}")
